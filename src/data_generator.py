...
def parse_dot_content(dot_content):
    ...
    # 匹配节点信息
    node_pattern = re.compile(r'\"(\d+)\"\s*\[label\s*=\s*<\((.*?)\,(.*?)\)<SUB>(\d+)</SUB>>\s*\]', re.DOTALL)
    for node_match in node_pattern.finditer(dot_content):
        node_id = node_match.group(1)
        node_type = html.unescape(node_match.group(2))
        node_code = html.unescape(node_match.group(3))
        line_number = int(node_match.group(4))
        nodes_info.append({
            "id": node_id,
            "type": node_type,
            "code": node_code,
    ...
    # 匹配边信息
    edge_pattern = re.compile(r'\"(\d+)\"\s*->\s*\"(\d+)\"\s*\[ label\s*=\s*\"(.*?)\"\]', re.DOTALL)
    for edge_match in edge_pattern.finditer(dot_content):
        source = edge_match.group(1)
        destination = edge_match.group(2)
        label = html.unescape(edge_match.group(3))
        edges_info.append({
            "source": source,
...
def build_PDG(sensi_api_path: str, dot_path: str) -> Tuple[nx.DiGraph, Dict[str, Set[int]]]:
    """
    build program dependence graph from code

    Args:
        sensi_api_path (str): path to sensitive apis
        dot_path (str): dot file path
    ...
    for edge in edges:
        ...
        if start_ln != end_ln:
            if edge_type[:3] == 'CDG':  # Control
                control_edges.add((start_ln, end_ln))
            elif edge_type[:3] == 'DDG':  # Data
                data_edges.add((start_ln, end_ln))
    control_edges_list = list(control_edges)
    data_edges_list = list(data_edges)
    PDG.add_edges_from(control_edges_list)
    PDG.add_edges_from(data_edges_list)
    return PDG, {
        "call": call_lines,
        "array": array_lines,
        "ptr": ptr_lines,
...
def build_IDG(code_path: str, PDG: nx.DiGraph, key_line_map: Dict[str, Set[int]],
    ...
    for key in ["call", "array", "ptr", "arith"]:
        for ln in key_line_map[key]:
            ...
            if len(sliced_lines) != 0:
                IDG = PDG.subgraph(list(sliced_lines)).copy()
                IDG.graph["key_line"] = ln
    ...
    with open(code_path, "r", encoding="utf-8") as f:
        lines = f.readlines()
    line_to_statement = dict()
    for idgs in res.values():
        for idg in idgs:
            for line in idg.nodes():
                line_to_statement[line] = lines[line-1].strip()

    return res, line_to_statement
...
def dump_IDG(res: Dict[str, List[nx.DiGraph]], out_root_path: str,
             testcaseid: str):
    """
    dump IDG to file
    ...
    for k in res:
        ...
        for IDG in res[k]:
            out_path = join(k_root_path, f"{IDG.graph['key_line']}.idg.pkl")
            nx.write_gpickle(IDG, out_path)
...
def handle_queue_message(queue: Queue):
    ...
    while True:
        ...
        if message.is_finished:
            break
        if message.IDG_res is not None:
            dump_IDG(message.IDG_res, message.out_root_path, message.testcaseid)
            for k in message.IDG_res:
                idg_ct += len(message.IDG_res[k])
    return idg_ct
    ...
    if testcaseid in codeIDtoPath:
        ...
        for file_path in file_map:
            ...
            PDG, key_line_map = build_PDG(csv_path, "data/sensiAPI.txt",
                                          source_path)
            res = build_IDG(PDG, key_line_map, vul_lines)
            queue.put(QueueMessage(res, out_root_path, testcaseid))
            # dump_IDG(res, out_root_path, testcaseid)
    return testcaseid
...
def generate(config_path: str):
    ...
    out_root_path = join(cwe_root, "IDG")
    ...
    with Manager() as m:
        ...
        pool = Pool(USE_CPU)
        idg_ct = pool.apply_async(handle_queue_message, (message_queue,))
...
if __name__ == "__main__":
    ...
    PDG, key_line_map = build_PDG(r"D:\pytorchProject\DeepWukong\data\sensiAPI.txt",
                                  r'D:\pytorchProject\VulExplain\1-pdg')
    idg_dict, line_to_statement = build_IDG(r'D:\pytorchProject\VulExplain\7785.c', PDG, key_line_map)
    print(idg_dict)
