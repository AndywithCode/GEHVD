...
def train(model: LightningModule, data_module: LightningDataModule, config: DictConfig):
    ...
    parts_nam = basename(config.encoder.path)+'_'+config.gnn.name
    # tensorboard logger
    tensorlogger = TensorBoardLogger(join("ts_logger", model_name),
                                     join(dataset_name, parts_nam))
    ...
    upload_weights = UploadCheckpointCallback(join(tensorlogger.log_dir, "checkpoints"))
